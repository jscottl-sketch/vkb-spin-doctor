# VKB Spin Doctor — Project Handover v19 (MASTER)
...
**Last updated:** 17 May 2026 (handover maintenance: ACCA codes, GitHub, Cloudflare, Groq auth, Qwen2.5-VL-72B, file tree, credit burn warning)
**Consolidates:** v18
...
| Handover injection | Loads VKB_SpinDoctor_Handover_v29.md into system prompt on startup |
...
| Groq Llama 3.3 70B | groq/llama-3.3-70b-versatile | 2 (free) | ⚠️ Auth broken via Google OAuth — email magic link in Edge (not Chrome) at console.groq.com confirmed working |
| Groq DeepSeek R1 | groq/deepseek-r1-distill-llama-70b | 2 (free) | ⚠️ Auth broken via Google OAuth — email magic link in Edge (not Chrome) at console.groq.com confirmed working |
| Gemini 2.5 Flash | gemini/gemini-2.5-flash | 2 (free) | ✅ Working |
| Mistral Codestral | mistral/codestral-latest | 2 (free) | ✅ Working |
| Cloudflare Workers AI | cloudflare/workers-ai | 2 (free) | ⚠️ Key lost — re-login dash.cloudflare.com |
...
├── aafl_core.py                       # Provider routing — 14 providers, cheapest-first
...
├── HOW_TO_INTEGRATE_DIAGNOSTIC.py     # Integration guide for screenshot diagnostic
...
├── VKB_SpinDoctor_Handover_v29.md     # This file — read by sfl_agent on startup
├── problems/
│   ├── __init__.py
...
| Balance | $20.00 loaded 12/05/2026 — partial burn from debug loop, check console.anthropic.com for current balance |
...
| Agent guesses wrong path | Path injection is in v3 — if broken, check VKB_SpinDoctor_Handover_v29.md is present |
...
| Groq auth fails | Google OAuth broken — use email magic link in Edge (not Chrome) at console.groq.com |
| Cloudflare key missing | Key lost — re-login at dash.cloudflare.com to get new Workers AI token |
