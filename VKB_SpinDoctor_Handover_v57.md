# VKB Spin Doctor — Project Handover v57 (MASTER)

**Owner:** Scott (Croydon, England)
**Status:** OCB-J built — HC-01–HC-10 system health checks across 3 files, Safety Shield panel (6 pills + auto-poll), CLACHR Relay task dispatch, AFNA/CLACHR API endpoints, ACCA update. 108/108 MOT ALL CLEAR.
...
| mission_control.html — Central Command (MCC) | ✅ Upgraded — 9+ tabs + Health Suite sub-tabs. Self-Health Settings sub-tab added OCB-A. mcc-popup-safe CSS class added. |
...
| Health Suite | mcc_server.py /api/self-health/* | Provider Health, Self-Diagnosis, AAFL & Scout Runs, GPU/CPU/RAM, Medical, Work Checker, ⚙️ Self-Health |

### Health Suite — Sub-Tabs

| Sub-Tab | What it shows |
|---|---|
| Provider Health | Live ping + success rate per AI provider |
| Self-Diagnosis | MCC component health status |
| AAFL & Scout Runs | Last N AAFL + Scout run summaries |
| GPU/CPU/RAM | Hardware utilisation metrics |
| Medical | mcc_medical.py 285-check deep diagnostic |
| Work Checker | Background work verification |
| ⚙️ Self-Health | Settings: Test Frequency, Auto-Fix Permission, Failure Escalation, Current Status |

### Self-Health System (OCB-A)

| Component | What it does |
|---|---|
| self_health.py | SelfHealthRunner — loads element_registry.json, tests each element via HTTP, stores pass/warn/fail in health.db |
| data/element_registry.json | 125 elements: id, name, tab, type, endpoint, method, healthy_when, severity_if_broken, auto_fix_id |
| data/health.db | SQLite — health_results (element_id, status, response_ms, error, run_id) + health_runs (score, trigger_source, counts) |
| data/solution_database.json | 12 solutions fix_001–fix_012 — match_pattern, fix_steps[], success_rate, user_approval_required |
| data/self_health_config.json | frequency_modes, auto_fix_mode, escalate_to_stuck_inbox, last_run, next_scheduled |

**Element types:** button | data_field | graph | endpoint | toggle | input

**Severity levels:** low | medium | high | critical

**Test logic:** button (POST 200=pass, 400/422=warn, other=fail) | data_field/endpoint (GET 200+non-empty=pass) | graph (empty array=warn) | toggle (GET 200=pass) | input (always pass/skip)

**OCB-B (deferred):** Body Map clinical visual + Auto-Fix run engine + Real-Time status streaming

### Self-Health — MCC Endpoints (mcc_server.py)

| Endpoint | Method | What it does |
|---|---|---|
| /api/self-health/registry | GET | Returns full element_registry.json |
| /api/self-health/last-run | GET | Returns latest health_run row from health.db |
| /api/self-health/config | GET | Returns self_health_config.json |
| /api/self-health/config | POST | Saves config updates atomically (.tmp + replace) |
| /api/self-health/solutions | GET | Returns solution_database.json |
| /api/self-health/history | GET | Returns last 20 health_runs from health.db |
| /api/self-health/run | POST | Imports SelfHealthRunner, calls run_all(trigger_source) |
| /api/self-health/run-tab | POST | Calls run_by_tab(tab_name) |
...
| Handover injection | Loads STATUS.md into system prompt on startup |
...
├── VKB_SpinDoctor_Handover_v57.md     # This file — read by sfl_agent on startup
├── problems/
│   ├── __init__.py
...
│   ├── health.db                      # SQLite — health_results + health_runs tables (auto-created on first run)
...
├── mission_control.html               # Central Command (MCC) — 9+ tabs inc Health Suite, Self-Health Settings sub-tab, mcc-popup-safe CSS
...
| Agent guesses wrong path | Path injection is in v3 — if broken, check VKB_SpinDoctor_Handover_v57.md is present |
...
> "Continuing VKB Spin Doctor. Read VKB_SpinDoctor_Handover_v57.md. OCB-J built — HC-01–HC-10 system health checks (self_health.py run_checks(), system_monitor.py get_full_snapshot(), work_checker.py generate_report()), Safety Shield panel on Home tab (6 pills, auto-polls /api/safety-status every 15s, green pulse / red flash badge), CLACHR Relay replaces Task Inbox (Dispatch All + live queue + results panel + Copy Results), /api/clachr/* and /api/stuck/afna-suggestions endpoints added, ACCA.md CLACHR entry appended, meta_proposals/SUMMARY.md decision table written. 108/108 MOT ALL CLEAR. AAFL competes with LangGraph/CrewAI/AutoGPT. Star Citizen v0.2 = first public benchmark + post trigger. Next: upload mcc-instructions-keeper skill, OCB-B (Body Map + Auto-Fix)."
...
**New ACCA codes:** None
**Ideas discussed:** Section-level IDs (section_wccs, section_scout_swarm etc.) added to instructions_db.json as a convention — allows ? buttons at tab level without requiring new element_registry entries. The skill file tells future Claude sessions exactly how to keep instructions_db.json in sync as new elements are added.
**Bugs fixed:** None
**Next priorities:**
1. Upload skills/mcc-instructions-keeper/SKILL.md to Project Files on claude.ai
