...
| OCB-M Phase 3 (GPU N/A verify+fix) | ✅ GPU drill-down condition fixed (was checking !d.gpu_name which is always "Unknown"). N/A state now resets needle + dasharray. N/A confirmed correct when no NVIDIA GPU. |
| OCB-M Phase 4 (Help tab verify) | ✅ 🔍 Help tab confirmed in tab bar. /api/help/ask endpoint confirmed responding. No changes needed. |
| OCB-M Phase 5 (Pie chart navigation) | ✅ Storage pie segments clickable — click any slice to scroll to + highlight that slot's detail card. storScrollToSlot() added. Slot cards given anchor IDs. |
| OCB-M Phase 6 (AI providers as LELs) | ✅ 11 AI providers added to data/llow_elements.json as ai_providers category. Tier badges (local/free/paid). Strength/weakness in hover tooltip. LLOW_ZONE_CATS updated. |
| OCB-M Phase 7 (Health Suite drill-downs) | ✅ Patient Fit for Service expandable panel (last 10 scores, trend, worst component, recommended action). Score history bars clickable — popup shows provider/goal/score detail. |
| OCB-M Phase 8 (Instructions restructure) | ✅ Instructions tab restructured into 3 collapsible accordion sections: INFORMATION / INSTRUCTIONS / CODES. All existing content preserved and reorganised. |
| OCB-M Phase 9 (AI Appendix) | ✅ Full AI provider comparison table (sortable by name/speed/quality/cost/tier) + radar/spider charts (5 axes: Speed/Code/Reasoning/Cost Efficiency/Reliability) for all 11 providers inside INFORMATION section. |
...
| Help | /api/help/ask (SSE) | AI-powered Q&A using project-aware free providers |

### LLOW Canvas (OCB-M new features)

| Feature | What it does |
|---|---|
| Zone headers (OCB-M) | INPUT / PROCESS / OUTPUT labels always visible at top of canvas |
| LEL dblclick (OCB-M fix) | Manual detection — double-click LEL on canvas opens options popup reliably |
| AI PROVIDERS category (OCB-M) | 11 AI providers as draggable LELs with tier badges and strength/weakness tooltips |
| LLC = Loop Law Chain (OCB-M) | Chain of AI provider LELs — context passes from node to node through the chain |

### AI Appendix (OCB-M Phase 9)

Full comparison of all 11 AI providers in Instructions > INFORMATION > AI Appendix:
- Sortable table: name, tier, model, cost, speed, quality, best uses (top 3), worst uses, recommended for, avoid for
- Radar charts: 5 axes per provider — Speed / Code Quality / Reasoning / Cost Efficiency / Reliability
...
├── mission_control.html               # Central Command (MCC) — OCB-M: zone headers, LEL dblclick fix, AI providers, Instructions restructure, AI Appendix
