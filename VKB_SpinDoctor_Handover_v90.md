# VKB Spin Doctor — Project Handover v90 (MASTER)

**Owner:** Scott (Croydon, England)
**Status:** v90 — OCB skip-mot flag (non-HTML tasks bypass MOT); loop output JSON fix (82 garbage .py files archived, code blocks/results now .json); learning machine confirmed 92 docs/537 chunks; 109/109 MOT ALL CLEAR.
...
| mission_control.html — Central Command (MCC) | ✅ v90 — Skip MOT checkbox in OCB Runner toolbar (id=ocb-skip-mot) |
...
| mcc_server.py | ✅ v90 — skip_mot field in POST /api/ocb/run body |
...
| HITSAV S11: OCB Runner v4 | ✅ v90 — Skip MOT checkbox (non-HTML tasks); Clear Lock button |
...
| OCB Runner: skip_mot flag | ✅ NEW v90 — run_safe(skip_mot=False) param; when True, CHECK D (MOT) skipped entirely |
| OCB Runner: skip_mot MCC field | ✅ NEW v90 — POST /api/ocb/run accepts {"skip_mot": true} in JSON body |
| OCB Runner: Skip MOT checkbox | ✅ NEW v90 — label+checkbox id=ocb-skip-mot in OCB Runner toolbar; ocbRun() sends skip_mot |
| loop_manager.py: JSON-only output | ✅ NEW v90 — code blocks saved as .json (not .py); results saved as _result.json (not _result.txt) |
| archive_dead/loop_garbage/ | ✅ NEW v90 — 82 garbage *_not_json_at_all*.py files moved from loop_output/ |
...
### Core Tabs — v90 (7 Primary + Sidebar)
...
VKB-SpinDoctor/
├── spin_doctor.py
├── sfl_agent.py                       # v3 — HANDOVER_FILENAME → v90
...
├── clacker_router.py                  # v71 — classify() + classify_all()
├── ocb_runner.py                      # v90 — run_safe(skip_mot=False); skip_mot bypasses CHECK D
...
├── mcc_server.py                      # v90 — POST /api/ocb/run reads skip_mot from JSON body
├── mcc_full_mot.py                    # 109 tests, GROUP A-H
├── mission_control.html               # v90 — Skip MOT checkbox (id=ocb-skip-mot) in OCB Runner toolbar
├── CLAUDE.md
├── ACCA.md
├── ALP_Database.md
├── STATUS.md
...
├── aafl_config.json
├── VKB_SpinDoctor_Handover_v90.md     # This file
├── learning_machine/                  # v89 — learning machine package
│   ├── __init__.py
│   ├── rag_engine.py                  # ChromaDB RAG; reindex_all/query/index_file; 92 docs 537 chunks (v90)
...
└── archive_dead/
    └── loop_garbage/                  # NEW v90 — 82 *_not_json_at_all*.py files archived here
```
...
> "Continuing VKB Spin Doctor. Read VKB_SpinDoctor_Handover_v90.md. v90 session — Three fixes: (1) OCB Runner skip-mot flag: run_safe() now has skip_mot=False param; when True, CHECK D (MOT) is skipped entirely — useful for Python scripts, API calls, colour changes where HTML MOT is irrelevant. mcc_server.py reads skip_mot from POST /api/ocb/run JSON body. mission_control.html has 'Skip MOT' checkbox (id=ocb-skip-mot) next to Parse button; ocbRun() sends skip_mot in request. (2) Loop output JSON fix: loop_manager.py code blocks now saved as .json wrapping raw content (not .py); result saves changed from _result.txt to _result.json. 82 existing *_not_json_at_all*.py garbage files moved to archive_dead/loop_garbage/. (3) Learning machine from v89 confirmed: reindex_all() produces 92 docs / 537 chunks; all 5 test scripts PASS (5/5); RAG wired into aafl_core.py; after_run() wired into loop_manager.py. 109/109 MOT ALL CLEAR throughout."
