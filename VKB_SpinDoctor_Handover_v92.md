# VKB Spin Doctor — Project Handover v92 (MASTER)

**Owner:** Scott (Croydon, England)
**Status:** v92 — ANCOREG tab (system doctor: diagnosis/fix-db/save-validator/MOT-dashboard/error-timeline) + BSTP tab (brainstorm planner: drag-drop canvas + 5 element types + action launcher + instruction pad + plan history) + ACCA codes JSON export (41 codes, GET /api/acca/codes, POST /api/acca/add) + ancoreg_validator.py (8 save-system checks) + 15 new server endpoints; 109/109 MOT ALL CLEAR.
...
### Core Tabs — v92 (9 Primary + Sidebar)

| Tab | Where | What it shows |
|---|---|---|
| HITSAV | **Top bar** | 13-section accordion: all save/ideas/history/OCB tools |
| ANCOREG | **Top bar** | System Doctor — diagnosis/fix DB/save validator/MOT dashboard/error timeline |
| BSTP | **Top bar** | Brainstorm Planner — drag-drop canvas + 5 element types + action launcher |
...
VKB-SpinDoctor/
├── spin_doctor.py
├── sfl_agent.py                       # v3 — HANDOVER_FILENAME → v92
...
├── mcc_server.py                      # v92 — 15 new endpoints: /api/acca/codes+add, /api/ancoreg/*, /api/bstp/*
├── mcc_full_mot.py                    # 109 tests, GROUP A-H
├── mission_control.html               # v92 — ANCOREG tab + BSTP tab added; JS injected before MOT stubs
├── CLAUDE.md
├── ACCA.md
├── ALP_Database.md
├── STATUS.md
...
├── aafl_config.json
├── VKB_SpinDoctor_Handover_v92.md     # This file
...
> "Continuing VKB Spin Doctor. Read VKB_SpinDoctor_Handover_v92.md. v92 session — Three major deliverables: (1) ACCA knowledge export — data/acca_codes.json with 41 codes; GET /api/acca/codes; POST /api/acca/add (atomic write). (2) ANCOREG tab (Autonomous Node Condition Registry) — System Doctor: Section 1 Live Diagnosis (health.db + detective report, 60s cache, auto-runs on tab open), Section 2 Fix Database (solution_database.json table + search + Add Fix modal), Section 3 Save Validator (8 checks via ancoreg_validator.py: STATUS line count, 4x END_OF_FILE, freshness, git status, split integrity, aafl_wccs importable, session_logs present), Section 4 MOT Dashboard (colour grid + click-to-detail), Section 5 Error Timeline (horizontal scroll from solution_database). 7 new /api/ancoreg/* endpoints. ancoreg_validator.py created (standalone 8-check script). (3) BSTP tab (Brainstorm Planner) — drag-drop canvas: 5 element types (TASK/DECISION/MILESTONE/SPLITTER/NOTE), SVG connectors, properties panel, undo/redo, zoom, fullscreen, context menu (send to AAFL/Kanban/CLAC). Control panel: Section A STATUS.md toggle (default OFF, preview, backup), Section B Instruction Pad, Section C Action Launcher, Section D Plan History. 8 new /api/bstp/* endpoints. data/bstp_plans/ created. instructions_db.json updated with ancoreg_tab + bstp_tab entries. 109/109 MOT ALL CLEAR throughout."
