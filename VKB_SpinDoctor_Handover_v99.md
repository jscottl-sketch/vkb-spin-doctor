# VKB Spin Doctor — Project Handover v99 (MASTER)

**Owner:** Scott (Croydon, England)
**Status:** v99 — HITSAV save+copy button feedback fixed (clean load-listener replaces double-bound inline onclick+IIFE); OCB Runner encoding+false-abort fixes; 109/109 MOT ALL CLEAR.
...
| CCR | Claude Chat Relay — dispatches free provider tasks from Claude Chat via MCC |
| + | Combine codes |
| = | Define a new code |
...
| sfl_agent.py v3 — ACP v1 + handover injection | ✅ ~920 lines, call_aafl() added; HANDOVER_FILENAME → v99 |
...
| mission_control.html — Central Command (MCC) | ✅ v99 — HITSAV save+copy button feedback: clean load-listener, fetch→visual state; inline onclick + IIFE double-binding removed |
...
| mcc_server.py | ✅ v93 — 15 new endpoints: /api/acca/codes, /api/acca/add, /api/ancoreg/*, /api/bstp/* |
...
| HITSAV sticky toolbar: 3-step save flow | ✅ v94 — ② Copy STATUS.md button no longer disabled; always pressable |
...
| OCB Runner Parse button — ghost style | ✅ v94 — border: 2px solid #5b8dee; background: transparent; color: #5b8dee; hover fills 15% |
| OCB Runner Run button — ghost style | ✅ v94 — border: 2px solid #3ecf8e; background: transparent; color: #3ecf8e; hover fills 15% |
| OCB Runner textarea placeholder | ✅ v94 — "Read STATUS.md. Count the lines in the BUILT AND WORKING table. Report the number only." |
| SpinDoctor Dark theme CSS | ✅ v94 — :root variables overridden (--bg-primary #0a0a18, --accent-purple #7c4dff, etc.); full selector block applies theme to tab panes / cards / accordions / inputs / tables / scrollbars / toasts / badges |
| mcc_settings.json active_theme | ✅ v94 — "active_theme": "SpinDoctor Dark" saved to data/mcc_settings.json |
...
| SpinDoctor Dark theme override | ✅ v94 — second :root block + comprehensive CSS selector block; overrides Project Brain values |
...
| data/acca_codes.json | ✅ v92 — 41 ACCA codes exported; GET /api/acca/codes; POST /api/acca/add (atomic) |
| ancoreg_validator.py | ✅ v92 — 8 save-system checks: STATUS line count, 4x END_OF_FILE markers, freshness, git clean, split integrity, aafl_wccs importable, session_logs present |
| ANCOREG tab (MCC) | ✅ v92 — data-tab="ancoreg"; 5 sections: Live Diagnosis / Fix Database / Save Validator / MOT Dashboard / Error Timeline |
| ANCOREG Section 1: Live Diagnosis | ✅ v92 — ancoregRunDiagnosis() calls /api/ancoreg/diagnosis; auto-runs on tab open (60s cache); card grid grouped by area |
| ANCOREG Section 2: Fix Database | ✅ v92 — table from solution_database.json; search filter; + Add Fix modal; clickable row expand |
| ANCOREG Section 3: Save Validator | ✅ v92 — ancoregValidateSave() → /api/ancoreg/validate-save → ancoreg_validator.py; Run Save Now → /api/ancoreg/run-save; Preview Diff |
| ANCOREG Section 4: MOT Dashboard | ✅ v92 — ancoregRunMOT() → /api/ancoreg/mot; colour grid 14×14px squares; click square → name+detail |
| ANCOREG Section 5: Error Timeline | ✅ v92 — ancoregLoadTimeline() → /api/ancoreg/error-timeline; horizontal scroll; click node → alert |
| BSTP tab (MCC) | ✅ v92 — data-tab="bstp"; drag-drop canvas + left palette + right properties panel |
| BSTP: 5 element types | ✅ v92 — TASK (blue), DECISION (yellow), MILESTONE (green), SPLITTER (orange), NOTE (grey dashed) |
| BSTP: canvas toolbar | ✅ v92 — Save / Load / Clear / Undo / Redo / Zoom +/- / Fullscreen |
| BSTP: connector drawing | ✅ v92 — click port circle → click target element → arrow drawn; SVG rendered via bstpRenderSVG() |
| BSTP: context menu (right-click) | ✅ v92 — Send to AAFL / Send to Kanban / Send to CLAC / Delete |
| BSTP Control A: STATUS.md Toggle | ✅ v92 — toggle switch (default OFF); preview shows what would be written; POST /api/bstp/status-write |
| BSTP Control B: Instruction Pad | ✅ v92 — textarea; Save → POST /api/bstp/instructions; loads on tab open |
| BSTP Control C: Action Launcher | ✅ v92 — bstpLaunchSelected() → AAFL / Kanban / CLAC; POST /api/bstp/action |
| BSTP Control D: Plan History | ✅ v92 — load all snapshots; click to load plan |
| /api/ancoreg/diagnosis GET | ✅ v92 — health.db last results + detective_report.json findings |
| /api/ancoreg/fixes GET | ✅ v92 — returns solution_database.json |
| /api/ancoreg/add-fix POST | ✅ v92 — adds entry to solution_database.json (atomic) |
| /api/ancoreg/validate-save GET | ✅ v92 — runs ancoreg_validator.validate_save_system() |
| /api/ancoreg/run-save POST | ✅ v92 — subprocess aafl_wccs.py |
| /api/ancoreg/mot GET | ✅ v92 — runs mcc_full_mot.py; returns full_mot_report.json |
| /api/ancoreg/error-timeline GET | ✅ v92 — timeline from solution_database.json sorted by date |
| /api/acca/codes GET | ✅ v92 — returns data/acca_codes.json with count |
| /api/acca/add POST | ✅ v92 — adds code entry (atomic); requires code + full_name |
| /api/bstp/current GET | ✅ v92 — returns data/bstp_current.json |
| /api/bstp/save POST | ✅ v92 — saves current plan + snapshot in data/bstp_plans/ |
| /api/bstp/plans GET | ✅ v92 — returns data/bstp_history.json snapshot index |
| /api/bstp/plan/<id> GET | ✅ v92 — loads specific plan snapshot |
| /api/bstp/action POST | ✅ v92 — sends task to AAFL/Kanban/CLAC |
| /api/bstp/instructions GET | ✅ v92 — returns data/bstp_instructions.json |
| /api/bstp/instructions POST | ✅ v92 — saves instruction pad (atomic) |
| /api/bstp/status-write POST | ✅ v92 — preview or write task to STATUS.md (with .bak backup) |
| instructions_db.json ANCOREG+BSTP | ✅ v92 — ancoreg_tab + bstp_tab entries added with nested_topics |
| ANCOREG accordion (Health Suite) | ✅ v93 — ANCOREG Doctor 5 sections merged into Health Suite accordion; standalone ANCOREG top-bar tab removed |
| BSTP accordion (Missions) | ✅ v93 — BSTP Brainstorm Planner sections merged into Missions accordion; standalone BSTP top-bar tab removed |
| Mission Mode toggle (Missions) | ✅ v93 — Mission Mode toggle added to Missions tab |
| GRRICE Changelog section | ✅ v93 — Changelog accordion section added to GRRICE tab |
| ACCA info boxes | ✅ v93 — Info popup boxes added to ACCA Codes section in Instructions tab |
| WCCS direct session log | ✅ v93 — aafl_wccs.py writes session log directly without subprocess; speed fix applied |
| POST /api/acca/command | ✅ v93 — ACCA command endpoint: dispatches ACCA code actions from MCC |
| Toast audit: AI empty-response handling | ✅ v95 — hitsavAutoUpdateStatus: d.error containing "empty" now shows amber 'warn' toast "Provider returned nothing — try again" instead of red error |
| updateAiStatusBar load delay | ✅ v95 — setTimeout(updateAiStatusBar, 2000) → 10000ms; page renders cleanly before first provider-health fetch |
| ancoreg_observer.py — OCB run observer | ✅ NEW v96 — pre_run() snapshots MOT score + health.db FAIL count + STATUS.md line count; post_run() compares and writes CLEAN/REGRESSION/UNKNOWN verdict; _append_terminator() feeds terminator_feed.json on CLEAN verdict only |
| ocb_runner.py observer hooks | ✅ NEW v96 — pre_run called at start of run_safe() + run_ocb(); post_run called before return in run_safe() with full obj + live_output |
| GET /api/ocb/observer-log | ✅ NEW v96 — returns data/ocb_observer_log.json (keyed by run_id) |
| OCB Runner Observer panel (mission_control.html) | ✅ NEW v96 — small panel below OCB log; shows MOT before/after, errors before/after, CLEAN/REGRESSION/UNKNOWN verdict badge; "View full log ↗" link; loadOcbObserver() auto-called on terminal state |
| OCB Runner subprocess encoding fix | ✅ v98 — encoding=utf-8 on MOT subprocess call; (stdout or "")+(stderr or "") guards; TOTAL_CHECKS constant updated 108→109 |
| OCB Runner false-abort fix | ✅ v98 — data/ocb_abort.json added to .gitignore (stash was restoring abort=true); double-clear in run_safe(); MOT timeout 180s→60s; Lifeguard echo test PASS |
| HITSAV btn-wccs-hub + copyStatusBtn feedback | ✅ v99 — inline onclick stripped from both buttons; old IIFE (visual-only, no fetch) removed; clean window.load listener added with full fetch→visual-state for each button; 109/109 MOT ALL CLEAR |
...
### Core Tabs — v95 (7 Primary + Sidebar)

| Tab | Where | What it shows |
|---|---|---|
| HITSAV | **Top bar** | 13-section accordion: all save/ideas/history/OCB tools |
| OCB Runner | **Top bar** | Paste OCB block, parse + run with phase cards + ABORT + Clear Lock + Skip MOT checkbox. Ghost-styled Parse (#5b8dee) + Run (#3ecf8e) buttons |
...
### SpinDoctor Dark Theme (v94)

CSS variables in second `:root` block override Project Brain palette:
- `--bg-primary: #0a0a18` — very dark navy/black tab backgrounds
- `--bg-card: #16163a` — dark purple cards/panels/accordions
- `--accent-purple: #7c4dff` — primary action colour
- `--accent-orange: #ff9800` — all section/panel headers
- `--accent-blue: #5b8dee` — OCB Parse button + count badges
- `--accent-green: #00e676` — ACTIVE badges
- `--accent-cyan: #00bcd4` — fork-chosen items
- `--border-subtle: rgba(92,100,180,0.25)` — card borders

### Toast Behaviour (v95)

| Scenario | Toast type |
|---|---|
| AI provider gave empty response (user button click) | Amber warning: "Provider returned nothing — try again" |
| Network/server error (user button click) | Red error: "❌ Error: …" |
| Auto-firing page-load calls | Silent — no toast; console.warn only if relevant |
| updateAiStatusBar first load | Delayed 10s after page load (was 2s) |
...
VKB-SpinDoctor/
├── spin_doctor.py
├── sfl_agent.py                       # v3 — HANDOVER_FILENAME → v99
...
├── clacker_router.py                  # v71 — classify() + classify_all()
├── ocb_runner.py                      # v96 — pre/post ancoreg_observer hooks in run_safe() + run_ocb()
...
├── ancoreg_validator.py               # v92 — 8 save-system checks; validate_save_system() → dict
├── ancoreg_observer.py                # v96 — pre_run/post_run OCB hooks; CLEAN/REGRESSION/UNKNOWN verdict; terminator_feed
...
├── mcc_server.py                      # v96 — GET /api/ocb/observer-log added
├── mcc_full_mot.py                    # 109 tests, GROUP A-H
├── mission_control.html               # v99 — btn-wccs-hub + copyStatusBtn: inline onclick stripped; clean window.load listener; fetch→visual feedback
├── CLAUDE.md
├── ACCA.md
├── ALP_Database.md
├── STATUS.md
...
├── aafl_config.json
├── VKB_SpinDoctor_Handover_v99.md     # This file
...
│   ├── acca_codes.json                # v92 — 41 ACCA codes; {codes:{...}, version, last_updated}
│   ├── bstp_current.json              # v92 — current active BSTP plan
│   ├── bstp_history.json              # v92 — snapshot index {snapshots:[{file,saved_at,name,version}]}
│   ├── bstp_instructions.json         # v92 — Scott's instruction pad content
│   ├── bstp_plans/                    # v92 — versioned plan snapshots (plan_<timestamp>.json)
│   ├── ocb_status.json
│   ├── ocb_abort.json
│   ├── ocb_progress.json              # v91 — written atomically by _write_status; {phases, percent, status}
│   ├── ocb_observer_log.json          # v96 — keyed by run_id; {pre:{mot_score,error_count,status_linecount}, post:{...}, verdict}
│   ├── terminator_feed.json           # v96 — list of CLEAN OCB runs {timestamp,run_id,task,result,log_summary}
│   ├── ocb_queue.json
...
│   ├── llow_elements.json
│   ├── llow_workflows/
│   ├── mcc_settings.json              # v94 — "active_theme": "SpinDoctor Dark" added
...
| Auto-Update STATUS.md fails | Free providers are down — check Health Suite tab. Toast now shows amber warning (not red) |
...
| MCC looks different after v94 | SpinDoctor Dark theme is now applied — deep navy/purple palette. Hard-refresh (Ctrl+Shift+R) if colours look wrong. |
| AI Status Bar shows "Loading providers…" for 10s | Normal after v95 — load delay extended from 2s to 10s so page renders fully first |
...
- Don't override the SpinDoctor Dark CSS :root block in inline styles without testing — the cascade order matters
...
> "Continuing VKB Spin Doctor. Read VKB_SpinDoctor_Handover_v99.md. v98: OCB Runner encoding fix (utf-8 subprocess, stdout/stderr guards, TOTAL_CHECKS 108→109) + false-abort fix (ocb_abort.json gitignored, double-clear, MOT timeout 60s). v99: HITSAV save+copy buttons now give instant feedback — inline onclick+IIFE stripped, clean window.load listener added. MOT 109/109 ALL CLEAR. NEXT: re-enable AAFL_Overnight_Task, build CCR."
...
### 2026-06-02 (Claude Code session 8)
**Key decisions:** Three UI fixes to mission_control.html: (1) HITSAV sticky toolbar ② Copy STATUS.md button — removed `disabled` attribute; hitsavStep2() already fetches /api/status, copies data.content to clipboard via navigator.clipboard.writeText(), shows toast "STATUS.md copied to clipboard ✅" on success / "Copy failed — is MCC server running?" on error. Button is now always pressable. (2) OCB Runner tab Parse button restyled as ghost/outline (border: 2px solid #5b8dee; background: transparent; color: #5b8dee; hover fills rgba 15%); Run button restyled as ghost/outline (border: 2px solid #3ecf8e; background: transparent; color: #3ecf8e; hover fills rgba 15%). Both get transition: all 0.2s ease. OCB2 textarea placeholder updated to "Read STATUS.md. Count the lines in the BUILT AND WORKING table. Report the number only." (3) SpinDoctor Dark CSS theme applied — new :root block overrides Project Brain palette with --bg-primary #0a0a18, --bg-card #16163a, --accent-purple #7c4dff, --accent-orange #ff9800, --accent-blue #5b8dee, --accent-cyan #00bcd4, --accent-red #ff5252, --border-subtle rgba(92,100,180,0.25). Comprehensive selector block covers tab panes, accordion headers/bodies, inputs, tables, toasts, progress bars, scrollbars, count badges. AI status bar and sidebar deliberately excluded. mcc_settings.json: "active_theme": "SpinDoctor Dark" saved. 109/109 MOT ALL CLEAR.
**New ACCA codes:** None
**Bugs fixed:** HITSAV ② Copy STATUS.md button permanently disabled — blocked by HTML disabled attribute, not by logic.
...
### 2026-06-02 (Claude Code session 9)
**Key decisions:** Toast audit on mission_control.html. Searched all red toast/error notification paths for "empty response" and "AI provider gave empty response". Found: (1) hitsavAutoUpdateStatus() — when /api/mccm/generate-status returns {"error": "AI provider returned empty response"}, the d.error check at line 4713 fired a red toast. Fixed: error handler now checks if d.error contains 'empty' — if yes, shows amber warn toast "Provider returned nothing — try again" instead. Other errors still show red toast. (2) setTimeout(updateAiStatusBar, 2000) at line 17367 — this auto-fires the AI status bar provider-health fetch 2s after page load. Changed to 10000ms so the page renders fully before the first fetch fires. (3) Confirmed no /api/terminator/* endpoints exist in this codebase — that part of the spec was N/A. All other auto-firing page-load calls (hitsavDetectiveLoad 1.5s, loadToolbarTs 2s, _updateWatchdog 2s) were already silent on error — no toasts. 109/109 MOT ALL CLEAR.
**New ACCA codes:** None
**Bugs fixed:** hitsavAutoUpdateStatus showed red error toast for empty AI response — should be amber warning. updateAiStatusBar fired 2s after page load (within the 5s "fires cleanly" window).
**Ideas discussed:** None
**Next priorities:**
1. Re-enable AAFL_Overnight_Task once loop_output JSON bug confirmed fixed: schtasks /Change /TN "AAFL_Overnight_Task" /ENABLE
2. Build CCR (Claude Chat Relay) — CLACH→MCC→free provider dispatch circuit
...
### 2026-06-02 (Claude Code session 10)
**Key decisions:** ANCOREG Observer system built. (1) ancoreg_observer.py — new standalone module with pre_run(run_id, task_description) and post_run(run_id, result, log_output). pre_run snapshots current MOT score (from ocb_progress.json), health.db FAIL row count, STATUS.md line count — saves to data/ocb_observer_log.json keyed by run_id. post_run compares new state vs snapshot: REGRESSION if run failed/rolled_back/cancelled or if error_count increased; CLEAN if status=DONE or "all clear" in MOT score; UNKNOWN otherwise. (2) ocb_runner.py — pre_run hook added after initial _write_status() in run_safe(); post_run hook added before return obj. Task description extracted as joined phase names. run_ocb() also gets a pre_run call at CLI start. All hooks wrapped in try/except so they never break the OCB run. (3) Terminator feed — _append_terminator() called inside post_run on CLEAN verdict only; appends {timestamp, run_id, task, result:"pass", log_summary: first 200 chars of log} to data/terminator_feed.json; capped at 500 entries. (4) GET /api/ocb/observer-log endpoint added to mcc_server.py; route added in GET handler, handler function added after _handle_ocb_results. (5) mission_control.html — Observer panel added below OCB log wrap: shows MOT before/after, errors before/after, CLEAN (green)/REGRESSION (red)/UNKNOWN (amber) verdict badge, "View full log ↗" link to raw JSON. loadOcbObserver() JS function fetches /api/ocb/observer-log and updates panel with entry for current _ocbRunId. Called automatically when ocbPoll() hits a terminal state (DONE/PARTIAL/CANCELLED/FAILED). 109/109 MOT ALL CLEAR.
**New ACCA codes:** None
**Bugs fixed:** None (net-new build)
**Ideas discussed:** Observer log is the groundwork for TERMINATOR — the system that learns from successful OCB runs and eventually can auto-generate OCB blocks. Every CLEAN run adds a training example to terminator_feed.json showing what a successful build looks like.
**Next priorities:**
1. Re-enable AAFL_Overnight_Task once loop_output JSON bug confirmed fixed: schtasks /Change /TN "AAFL_Overnight_Task" /ENABLE
2. Build CCR (Claude Chat Relay) — CLACH→MCC→free provider dispatch circuit
3. Run a real AAFL loop (run_aafl.bat) — verify loop_output/*.json created
...
### 2026-06-02 (Claude Code session 11)
**Key decisions:** WCCS-only session — no new code built. Handover v96 consolidated into v97 to keep the save record current after the ANCOREG Observer build.
**New ACCA codes:** None
**Bugs fixed:** None
**Ideas discussed:** None
**Next priorities:**
...
### 2026-06-03 (Claude Code session 1)
**Key decisions:** OCB Runner two-part fix — (1) encoding fix: subprocess call for MOT now uses encoding='utf-8', (stdout or '')+(stderr or '') guards prevent NoneType crash, TOTAL_CHECKS constant updated 108→109 to match current MOT suite. (2) False-abort root cause found and fixed: data/ocb_abort.json had abort=true committed to git; stash pop during OCB test restored this file, triggering abort every run. Fix: ocb_abort.json added to .gitignore, double-clear logic added in run_safe(), MOT subprocess timeout tightened 180s→60s. Lifeguard echo test PASS.
**New ACCA codes:** None
**Bugs fixed:** OCB Runner always aborted after first run — committed ocb_abort.json (abort=true) restored by git stash. MOT score parser crashed on None stdout/stderr.
**Ideas discussed:** None
**Next priorities:**
1. Re-enable AAFL_Overnight_Task once loop_output JSON bug confirmed fixed
2. Build CCR (Claude Chat Relay)
3. Run a real AAFL loop (run_aafl.bat)
4. Complete STORM ↔ MCCM live loop testing
5. OCB-K Build 3 — Costs tab enhancements

---

### 2026-06-03 (Claude Code session 2)
**Key decisions:** HITSAV save+copy button feedback fix. Root cause: previous session added an IIFE that only changed visual state (text/background) but did not make any fetch call; meanwhile the inline onclick on each button also fired, causing double-binding with unpredictable visual result. Fix: stripped inline onclick from btn-wccs-hub and copyStatusBtn; removed the old IIFE entirely; added one clean window.addEventListener('load') block that assigns s.onclick and c.onclick each with full fetch→visual-state logic (⏳ Saving/Copying... → ✅ Saved/Copied or ❌ error). /api/hisav/save-session endpoint confirmed present in mcc_server.py (not added). Script block sits at line 22917, immediately before </body>. MOT 109/109 ALL CLEAR.
**New ACCA codes:** None
**Bugs fixed:** btn-wccs-hub and copyStatusBtn click produced no reliable visible feedback — double-bound handlers (inline onclick + IIFE addEventListener) competed and left buttons in inconsistent state.
**Ideas discussed:** None
**Next priorities:**
1. Re-enable AAFL_Overnight_Task once loop_output JSON bug confirmed fixed: schtasks /Change /TN "AAFL_Overnight_Task" /ENABLE
2. Build CCR (Claude Chat Relay) — CLACH→MCC→free provider dispatch circuit
