# VKB Spin Doctor — Project Handover v64 (MASTER)

**Owner:** Scott (Croydon, England)
**Status:** OCB-O Code Pipeline — Monaco Code Editor tab (file browser, run .py, CLAC generator), AAFL→Editor bridge, CLAC Generator panel, 3 new LLOW coding workflows (write_new_feature / fix_bug / refactor_file), 4 new /api/code/* endpoints. 108/108 MOT ALL CLEAR.
...
| mission_control.html — Central Command (MCC) | ✅ OCB-O Code Pipeline: Monaco Code Editor tab (file browser, run .py, CLAC generator, AAFL bridge, Open in LLOW), Code Task preset in AAFL Control, 3 new LLOW coding workflows |
...
| OCB-O Code Pipeline: Code Editor tab | ✅ Monaco Editor via CDN (0.44.0). File browser panel (expandable tree, lazy-load). Editor toolbar (Save/Format/Copy All/Run/Open in LLOW/Send to Claude Code/CLAC Panel). Output panel below editor (stdout/stderr, exit code). Ctrl+S to save. Unsaved changes indicator (● in tab title). Language auto-detected from extension. |
| OCB-O Code Pipeline: AAFL→Editor bridge | ✅ "Open in Code Editor" button appears in AAFL result pane when result contains a code block (```). Click extracts first code block, detects language, opens in Code Editor with filename suggestion. "Code Task" preset button in AAFL Control pre-fills structured code request prompt and routes to Mistral Codestral. |
| OCB-O Code Pipeline: CLAC Generator | ✅ Collapsible right panel in Code Editor. Task description → CLAC block (Read [file]. [task]. Write changes back to file.). One-click copy. Last 10 CLACs saved to localStorage. "Send to Claude Code" toolbar button also generates CLAC + copies instantly. |
| OCB-O Code Pipeline: Open in LLOW | ✅ Code Editor toolbar button opens AAFL Control → LLOW section, auto-loads fix_bug workflow for current file. |
| OCB-O Code Pipeline: LLOW coding workflows | ✅ 3 new workflow JSON files: write_new_feature.json (IBR→BPM→WRC→WCBB→SCORE_GATE→OCB→WCCS_END), fix_bug.json (IBR→BPM→WRC→SCORE_GATE→OCB→WCCS_END), refactor_file.json (IBR→BPM→WRC→WCBB→SCORE_GATE→OCB→WCCS_END). Auto-appear in LLOW Preset Load dropdown. |
| OCB-O Code Pipeline: /api/code/* endpoints | ✅ GET /api/code/files (directory tree, lazy-load, skips __pycache__/backups/archive_dead/.git), GET /api/code/read (file content, max 2MB, ext-detected), POST /api/code/save (atomic write via tempfile+rename), POST /api/code/run (subprocess Python, 30s timeout, stdout/stderr/returncode). All 4 monkey-patched onto MCCHandler. |
...
### Central Command — Core Tabs (OCB-O Code Pipeline: Code Editor tab added)
...
| AAFL Control | mcc_server.py /run-aafl | Goal control, provider dropdowns (14), loop settings, goal queue, live terminal output, run history. "Code Task" preset button routes to Mistral Codestral. |
| Code Editor | mcc_server.py /api/code/* | Monaco editor, file browser, run .py scripts, CLAC generator, AAFL bridge |
...
### Code Editor Tab (OCB-O Code Pipeline)

| Feature | Detail |
|---|---|
| Monaco Editor | CDN 0.44.0, dark theme, Ctrl+S to save, auto-layout, bracket colouring |
| File Browser | Left panel, expandable tree, lazy folder loading, skips __pycache__/backups/archive_dead |
| Language detection | Auto from file extension (.py=python, .js=javascript, .html=html, etc.) |
| Toolbar | Save (💾), Format (⚙), Copy All (📋), Run (▶ .py only), Open in LLOW (🔄), Send to Claude Code (📤), CLAC Panel (≡) |
| Output panel | Shows stdout/stderr + exit code after Run. Collapsible. |
| CLAC Generator | Collapsible right panel — type task, click Generate, copy 1-click. Last 10 CLACs in localStorage. |
| AAFL Bridge | "Open in Code Editor" button in AAFL result when result contains ``` code block. Extracts code, opens in editor. |
| Open in LLOW | Auto-loads fix_bug.json workflow for current file. |
| Right-click menu | Open / Rename / Delete (files); New File Here / Rename (folders) |
| New File | + button in file browser header. Prompt for name → empty editor. |

### LLOW Coding Workflows (OCB-O Code Pipeline)

| Workflow | Flow | Purpose |
|---|---|---|
| write_new_feature.json | IBR→BPM→WRC→WCBB→SCORE_GATE→[retry: AFNA→WRC]→OCB→WCCS_END | Research + plan + write + review + test → save |
| fix_bug.json | IBR→BPM→WRC→SCORE_GATE→[retry: AFNA→WRC]→OCB→WCCS_END | Paste error message → analyse → fix → test → save |
| refactor_file.json | IBR→BPM→WRC→WCBB→SCORE_GATE→[retry: AFNA→WRC]→OCB→WCCS_END | Analyse inefficiencies → rewrite cleanly → review → save |
...
├── VKB_SpinDoctor_Handover_v64.md     # This file — read by sfl_agent on startup
├── problems/
│   ├── __init__.py
...
│   └── llow_workflows/                # LLOW workflow presets (JSON)
│       ├── basic_research.json
│       ├── full_dev_cycle.json
│       ├── overnight_aafl.json
│       ├── bug_hunt.json
...
│       ├── scout_deep_dive.json
│       ├── star_citizen_benchmark.json
│       ├── tutorial_load.json
│       ├── write_new_feature.json     # OCB-O: Research→Plan→Write→Review→Test→Save code pipeline
│       ├── fix_bug.json               # OCB-O: Paste error→Analyse→Fix→Test→Save
│       └── refactor_file.json         # OCB-O: Analyse inefficiencies→Rewrite→Review→Save
├── skills/
│   └── mcc-instructions-keeper/
...
├── mission_control.html               # Central Command (MCC) — OCB-O Code Pipeline: Code Editor tab, LLOW workflows, CLAC generator
...
| Agent guesses wrong path | Path injection is in v3 — if broken, check VKB_SpinDoctor_Handover_v64.md is present |
...
| Code Editor tab blank / Monaco missing | Check network — Monaco loads from CDN. If offline, editor shows plain message. CDN: cdnjs.cloudflare.com/ajax/libs/monaco-editor/0.44.0 |
| Code Editor file tree empty | mcc_server.py must be running. Click ↻ to refresh. /api/code/files endpoint serves directory listing. |
| Code Editor Save fails | Check mcc_server.py is running on port 8080. POST /api/code/save uses atomic write. |
| Code Editor Run fails or times out | Script runs with 30s timeout. Check .py file path is saved first (Ctrl+S before Run). |
| LLOW coding workflow not in dropdown | Reload LLOW preset dropdown (click Refresh or re-open LLOW section). File must be in data/llow_workflows/. |
...
> "Continuing VKB Spin Doctor. Read VKB_SpinDoctor_Handover_v64.md. OCB-O Code Pipeline built — 5 phases: (1) Monaco Code Editor tab: Monaco 0.44.0 via CDN, file browser panel (expandable tree, lazy-load, skips __pycache__/backups/archive_dead), toolbar (Save/Format/Copy All/Run .py/Open in LLOW/Send to Claude Code/CLAC Panel), output panel (stdout/stderr/exit code), right-click menu (open/rename/new file). (2) AAFL→Editor bridge: 'Open in Code Editor' button appears in AAFL result pane when result contains a ``` code block — click extracts first block, detects language, opens in Code Editor with filename suggestion. 'Code Task' preset button in AAFL Control pre-fills structured prompt and routes to Mistral Codestral. (3) CLAC Generator: collapsible right panel, task description → CLAC block (Read [file]. [task]. Write changes back.), 1-click copy, last 10 CLACs to localStorage. 'Send to Claude Code' toolbar button generates + copies instantly. (4) 3 new LLOW coding workflows in data/llow_workflows/: write_new_feature.json, fix_bug.json, refactor_file.json — all auto-appear in LLOW Preset Load dropdown. 'Open in LLOW' toolbar button auto-loads fix_bug.json for current file. (5) 4 new mcc_server.py endpoints: GET /api/code/files (directory tree, lazy-load), GET /api/code/read (content, max 2MB), POST /api/code/save (atomic write), POST /api/code/run (subprocess Python, 30s timeout). 108/108 MOT ALL CLEAR. Next: OCB-B Body Map + Auto-Fix Engine, Star Citizen v0.2 benchmark."
...
**Key decisions:** OCB-O Code Pipeline built — 5 phases. (1) Monaco Code Editor tab: full tab added to MCC tab bar (after AAFL Control). Monaco 0.44.0 loaded via CDN (cdnjs.cloudflare.com). File browser left panel — expandable tree, lazy folder loading, skips __pycache__/backups/archive_dead/.git. Toolbar: Save (💾 Ctrl+S), Format (⚙), Copy All (📋), Run (▶ .py only, 30s timeout), Open in LLOW (🔄), Send to Claude Code (📤), CLAC Panel toggle (≡). Output panel below editor shows stdout/stderr + exit code. Unsaved changes indicator (● in tab title). Language auto-detected from extension. (2) AAFL → Code Editor bridge: "Open in Code Editor" button appears in AAFL result pane when result contains a ``` code block (complete status only). Click extracts first code block, detects language tag, opens in Code Editor with auto-generated filename. "Code Task" preset button in AAFL Control pre-fills structured prompt (filename comment + fenced code block requirement) and routes to Mistral. (3) CLAC Generator: collapsible right panel in Code Editor. Text area for task description. Generate button → "Read [file]. [task]. Write changes back to the file." One-click copy. Last 10 CLACs saved to localStorage + shown in history list. "Send to Claude Code" toolbar button generates CLAC directly with prompt then copies. (4) 3 new LLOW coding workflows: write_new_feature.json (IBR→BPM→WRC→WCBB→SCORE_GATE→[AFNA loop if <7]→OCB→WCCS_END), fix_bug.json (IBR→BPM→WRC→SCORE_GATE→[AFNA loop if <7]→OCB→WCCS_END), refactor_file.json (IBR→BPM→WRC→WCBB→SCORE_GATE→[AFNA loop if <7]→OCB→WCCS_END). Auto-appear in LLOW Preset Load dropdown. "Open in LLOW" toolbar button navigates to AAFL Control → LLOW, auto-loads fix_bug workflow. (5) 4 new mcc_server.py endpoints: GET /api/code/files (directory tree, lazy children, access-controlled to project root), GET /api/code/read (max 2MB, ext detected), POST /api/code/save (atomic via tempfile+rename), POST /api/code/run (subprocess Python, 30s timeout, stdout/stderr/returncode). All monkey-patched onto MCCHandler. 108/108 MOT ALL CLEAR.
**New ACCA codes:** None
**Bugs fixed:** None
**Ideas discussed:** Monaco as MCC's native code environment — AI writes code via AAFL, result auto-opens in editor, one-click save. CLAC Generator as an automation bridge — lets Scott generate "instructions for Claude Code" directly from within MCC without switching windows.
**Next priorities:**
1. OCB-B — Body Map visual + Auto-Fix run engine + Real-Time status streaming (Health Suite)
2. Star Citizen v0.2 benchmark via AAFL autonomous run
3. Add GROQ + Cloudflare keys to .env (manual — security rule)
