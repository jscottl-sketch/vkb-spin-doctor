# VKB Spin Doctor — Project Handover v89 (MASTER)

**Owner:** Scott (Croydon, England)
**Status:** v89 — Learning Machine built: RAG engine (ChromaDB, 91 docs/531 chunks), training data pipeline (alpaca JSONL), self-improving feedback loop; all wired into aafl_core.py + loop_manager.py; 7 new MCC endpoints; 5 OCB test scripts all PASS; 109/109 MOT ALL CLEAR.
...
| loop_manager.py — loop engine | ✅ v89 — feedback_loop.after_run() called at end of every AAFL iteration |
...
| mcc_server.py | ✅ v89 — 7 new /api/rag/* /api/training/* /api/learning/* endpoints |
...
| Learning Machine — RAG engine | ✅ NEW v89 — learning_machine/rag_engine.py; ChromaDB at data/rag_db/; 91 docs, 531 chunks; query/index_file/reindex_all |
| Learning Machine — Training Pipeline | ✅ NEW v89 — learning_machine/training_prep.py; scans loop_output/*.json; alpaca JSONL to data/training_data.jsonl |
| Learning Machine — Feedback Loop | ✅ NEW v89 — learning_machine/feedback_loop.py; after_run() routes high-score→RAG+training, low-score→failure_patterns.jsonl; learning report every 10 runs |
| RAG wired into aafl_core.py | ✅ NEW v89 — _rag_context() prepends top-3 chunks before every AI call; use_rag=True default, _RAG_ENABLED kill-switch |
| Feedback loop wired into loop_manager.py | ✅ NEW v89 — _feedback_after_run() called at end of every completed AAFL iteration |
| /api/rag/* endpoints (3) | ✅ NEW v89 — GET /api/rag/stats, POST /api/rag/reindex, POST /api/rag/query |
| /api/training/* endpoints (2) | ✅ NEW v89 — GET /api/training/stats, POST /api/training/rebuild |
| /api/learning/* endpoints (2) | ✅ NEW v89 — GET /api/learning/report, GET /api/learning/failures |
| OCB test scripts: t_rag_index.py | ✅ NEW v89 — PASS: 91 docs, 531 chunks |
| OCB test scripts: t_rag_query.py | ✅ NEW v89 — PASS: 3 results for 'spin doctor joystick fix' |
| OCB test scripts: t_training_scan.py | ✅ NEW v89 — PASS: 0 pairs (no JSON results yet) |
| OCB test scripts: t_feedback_report.py | ✅ NEW v89 — PASS: no runs yet |
| OCB test scripts: t_learning_full.py | ✅ NEW v89 — PASS: 4/4 steps |
...
### Core Tabs — v89 (7 Primary + Sidebar)
...
VKB-SpinDoctor/
├── spin_doctor.py
├── sfl_agent.py                       # v3 — HANDOVER_FILENAME → v89
├── aafl_core.py                       # v89 — RAG context injection before every AI call
├── loop_manager.py                    # v89 — feedback_loop.after_run() wired in
├── evaluator.py
├── researcher.py
├── memory_bank.py
├── cost_guard.py
...
├── clacker_router.py                  # v71 — classify() + classify_all()
├── ocb_runner.py                      # v88 — GUARD 1 stale lock auto-clear (>10min)
...
├── mcc_server.py                      # v89 — 7 new /api/rag+training+learning endpoints
├── mcc_full_mot.py                    # 109 tests, GROUP A-H
├── mission_control.html               # v88 — Clear Lock button; restored from git HEAD
├── CLAUDE.md
├── ACCA.md
├── ALP_Database.md
├── STATUS.md
...
├── aafl_config.json
├── VKB_SpinDoctor_Handover_v89.md     # This file
├── learning_machine/                  # NEW v89 — learning machine package
│   ├── __init__.py
│   ├── rag_engine.py                  # ChromaDB RAG; reindex_all/query/index_file; 91 docs 531 chunks
...
│   ├── knowledge_engine.db
│   ├── health.db
│   ├── rag_db/                        # NEW v89 — ChromaDB persistent store
│   ├── rag_stats.json                 # NEW v89 — {doc_count, chunk_count, last_indexed}
│   ├── training_data.jsonl            # NEW v89 — alpaca-format high-quality training pairs
│   ├── training_stats.json            # NEW v89 — {pair_count, total_runs, avg_score}
│   ├── failure_patterns.jsonl         # NEW v89 — low-score run failure log
│   ├── feedback_counter.json          # NEW v89 — running totals for learning report
│   ├── learning_report.json           # NEW v89 — latest generated learning report
│   ├── ocb_status.json
│   ├── ocb_abort.json
│   ├── ocb_progress.json
│   ├── ocb_queue.json
...
│       ├── t_rag_index.py                  # NEW v89 — RAG reindex test (PASS)
│       ├── t_rag_query.py                  # NEW v89 — RAG query test (PASS)
│       ├── t_training_scan.py              # NEW v89 — training scan test (PASS)
│       ├── t_feedback_report.py            # NEW v89 — feedback report test (PASS)
│       └── t_learning_full.py              # NEW v89 — full integration test (4/4 PASS)
└── archive_dead/
```
...
2. Run a real AAFL loop (run_aafl.bat) — verify feedback_loop.after_run() fires (look for [FEEDBACK] log lines)
3. After 10 runs — check data/learning_report.json for avg score trend + best provider
...
> "Continuing VKB Spin Doctor. Read VKB_SpinDoctor_Handover_v89.md. v89 session — Learning Machine built in 3 layers: (1) RAG engine (learning_machine/rag_engine.py) using ChromaDB stored at data/rag_db/ — indexes session_logs/*.md + STATUS/INDEX/HISTORY/ACCA + 4 data JSON files; 91 documents, 531 chunks; query() prepended into every aafl_core.py AI call via _rag_context(); use_rag=True default. (2) Training data pipeline (learning_machine/training_prep.py) — scans loop_output/*.json for score>=7 results, exports alpaca-format JSONL to data/training_data.jsonl. (3) Self-improving feedback loop (learning_machine/feedback_loop.py) — after_run() called from loop_manager.py after every AAFL iteration; high-score→RAG+training, low-score→failure_patterns.jsonl; learning report printed every 10 runs. 7 new MCC endpoints: GET/POST /api/rag/*, GET/POST /api/training/*, GET /api/learning/*. 5 OCB test scripts in tests/ocb_test_scripts/: all 5 PASS. 109/109 MOT ALL CLEAR."
