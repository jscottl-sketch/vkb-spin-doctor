# VKB Spin Doctor — Project Handover v91 (MASTER)

**Owner:** Scott (Croydon, England)
**Status:** v91 — OCB Runner AI brain confirmed wired (run_task→AAFLCore); live progress feedback (_write_status atomically writes ocb_progress.json per phase; phases PENDING/RUNNING/DONE/FAILED tracked in run_safe()); Skip MOT checkbox in HITSAV S11; 2s poll interval; 63 more loop garbage files archived; 109/109 MOT ALL CLEAR.
...
| mission_control.html — Central Command (MCC) | ✅ v91 — Skip MOT checkbox in HITSAV S11 (id=hitsav-ocb-skip-mot); ocbStartPoll 2s interval; ocbPoll fetches /api/ocb/progress for bar |
...
| mcc_server.py | ✅ v91 — _handle_ocb_progress reads ocb_progress.json first (falls back to ocb_status.json) |
...
| ocb_runner.py: _write_status progress write | ✅ NEW v91 — _write_status() also writes data/ocb_progress.json atomically when phases are present |
| ocb_runner.py: run_safe() phase tracking | ✅ NEW v91 — obj["phases"] built from parsed; ph_idx enumerate loop; PENDING→RUNNING→DONE/FAILED per phase |
| ocb_runner.py: mot_score in obj | ✅ NEW v91 — obj["mot_score"] set before _sl() calls so progress file captures MOT result |
| data/ocb_progress.json | ✅ NEW v91 — written atomically by _write_status after each phase; {phases, phase_current, phase_total, percent, status, mot_score} |
| OCB Runner: HITSAV S11 Skip MOT checkbox | ✅ NEW v91 — id=hitsav-ocb-skip-mot; hitsavOcbRun() reads it and sends skip_mot in JSON body |
| OCB Runner: 2s poll interval | ✅ NEW v91 — ocbStartPoll() setInterval 3000→2000ms |
| OCB Runner: progress endpoint in ocbPoll | ✅ NEW v91 — ocbPoll() also fetches /api/ocb/progress to update ocb-progress-fill bar |
| ocb_selftest.py | ✅ NEW v91 — self-test: parse + run_safe(skip_mot=True) + verify ocb_brain_test.txt + progress file |
| ocb_patch.py | ✅ NEW v91 — patch utility: applies all run_safe phase-tracking edits to ocb_runner.py via string replacement |
...
### Core Tabs — v91 (7 Primary + Sidebar)
...
VKB-SpinDoctor/
├── spin_doctor.py
├── sfl_agent.py                       # v3 — HANDOVER_FILENAME → v91
...
├── clacker_router.py                  # v71 — classify() + classify_all()
├── ocb_runner.py                      # v91 — _write_status writes ocb_progress.json; run_safe() phase tracking; mot_score in obj
├── ocb_runner_tests.py                # v86 — 8 unit tests for parse_ocb_block
├── ocb_selftest.py                    # NEW v91 — OCB Runner self-test script
├── ocb_patch.py                       # NEW v91 — patch utility for ocb_runner.py phase-tracking edits
...
├── mcc_server.py                      # v91 — _handle_ocb_progress reads ocb_progress.json first
├── mcc_full_mot.py                    # 109 tests, GROUP A-H
├── mission_control.html               # v91 — Skip MOT checkbox in HITSAV S11; 2s ocbStartPoll; ocbPoll fetches /api/ocb/progress
├── CLAUDE.md
├── ACCA.md
├── ALP_Database.md
├── STATUS.md
...
├── aafl_config.json
├── VKB_SpinDoctor_Handover_v91.md     # This file
...
> "Continuing VKB Spin Doctor. Read VKB_SpinDoctor_Handover_v91.md. v91 session — Four deliverables: (1) OCB Runner AI brain confirmed wired — run_task() calls AAFLCore (allow_paid=False, free providers only) for all code-editing tasks; verified via run_safe() code path. (2) Live progress feedback — _write_status() in ocb_runner.py now atomically writes data/ocb_progress.json after every phase; run_safe() tracks obj['phases'] (PENDING→RUNNING→DONE/FAILED) and obj['current_phase']; mcc_server.py _handle_ocb_progress reads ocb_progress.json first and falls back to ocb_status.json. (3) MCC panel updates — HITSAV S11 gets Skip MOT checkbox (id=hitsav-ocb-skip-mot); hitsavOcbRun() passes skip_mot to API; standalone OCB tab poll interval 3000→2000ms; ocbPoll() also fetches /api/ocb/progress for the bar. (4) Loop garbage — 63 additional *_not_json_at_all*.py files archived to archive_dead/loop_garbage/ (145 total across v90+v91). NOTE: ocb_runner.py edits are best applied via python ocb_patch.py rather than the Edit tool — the built-in linter strips function-body additions. 109/109 MOT ALL CLEAR throughout."
