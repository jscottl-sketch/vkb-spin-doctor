# VKB Spin Doctor — Project Handover v94 (MASTER)

**Owner:** Scott (Croydon, England)
**Status:** v94 — HITSAV Copy STATUS.md button un-disabled + OCB Runner Parse/Run ghost-button styling + SpinDoctor Dark purple theme CSS overhaul (new :root variables + comprehensive selector block); mcc_settings.json active_theme = "SpinDoctor Dark"; 109/109 MOT ALL CLEAR.
...
| mission_control.html — Central Command (MCC) | ✅ v94 — SpinDoctor Dark theme CSS; HITSAV Step 2 button un-disabled; OCB Runner ghost buttons |
...
| OCB Runner Parse button — ghost style | ✅ NEW v94 — border: 2px solid #5b8dee; background: transparent; color: #5b8dee; hover fills 15% |
| OCB Runner Run button — ghost style | ✅ NEW v94 — border: 2px solid #3ecf8e; background: transparent; color: #3ecf8e; hover fills 15% |
| OCB Runner textarea placeholder | ✅ NEW v94 — "Read STATUS.md. Count the lines in the BUILT AND WORKING table. Report the number only." |
| SpinDoctor Dark theme CSS | ✅ NEW v94 — :root variables overridden (--bg-primary #0a0a18, --accent-purple #7c4dff, etc.); full selector block applies theme to tab panes / cards / accordions / inputs / tables / scrollbars / toasts / badges |
| mcc_settings.json active_theme | ✅ NEW v94 — "active_theme": "SpinDoctor Dark" saved to data/mcc_settings.json |
...
| SpinDoctor Dark theme override | ✅ NEW v94 — second :root block + comprehensive CSS selector block; overrides Project Brain values |
...
### Core Tabs — v94 (7 Primary + Sidebar)
...
VKB-SpinDoctor/
├── spin_doctor.py
├── sfl_agent.py                       # v3 — HANDOVER_FILENAME → v94
...
├── mission_control.html               # v94 — SpinDoctor Dark theme; HITSAV Step 2 un-disabled; OCB ghost buttons
├── CLAUDE.md
├── ACCA.md
├── ALP_Database.md
├── STATUS.md
...
├── aafl_config.json
├── VKB_SpinDoctor_Handover_v94.md     # This file
...
> "Continuing VKB Spin Doctor. Read VKB_SpinDoctor_Handover_v94.md. v94 — Three UI fixes applied to mission_control.html: (1) HITSAV sticky toolbar ② Copy STATUS.md button un-disabled — was permanently greyed out; hitsavStep2() already fetches /api/status and copies to clipboard — just needed the disabled attr removed. (2) OCB Runner tab Parse button styled as ghost/outline (#5b8dee blue border, transparent background, 15% hover fill); Run button styled as ghost/outline (#3ecf8e green border); placeholder text updated. (3) SpinDoctor Dark theme CSS applied — new :root variable block overrides Project Brain palette with deep navy (#0a0a18) / dark purple (#16163a) / accent purple (#7c4dff); comprehensive selector block covers tab panes, cards, accordions, inputs, tables, toasts, badges, scrollbars, progress bars. mcc_settings.json active_theme = SpinDoctor Dark. 109/109 MOT ALL CLEAR. NEXT: re-enable AAFL_Overnight_Task, build CCR."
