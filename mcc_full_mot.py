...
# Fix Windows console Unicode issues (emojis in test messages)
try:
...
def test_group_h():
    ...
    # Safety net: handover files should not be sitting in root — WCCS archives them automatically
    handovers = sorted(HERE.glob("VKB_SpinDoctor_Handover_v*.md"))
    _pre("No handover files in root", "Checks root for VKB_SpinDoctor_Handover_v*.md — all should be in archive_dead/")
    if handovers:
        names = ", ".join(h.name for h in handovers[:3])
        extra = f" (+{len(handovers) - 3} more)" if len(handovers) > 3 else ""
        _record("H", "No handover files in root", True,
                f"⚠️ WARNING: {len(handovers)} handover file(s) in root — should be in archive_dead/: {names}{extra}")
    else:
        _record("H", "No handover files in root", True, "clean — none in root")
