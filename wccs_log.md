...
| 72 | 2026-05-31 | v77 -> v78 | Global tooltip visibility fix: all CSS z-index 9999→99999, tab bar z-index:100, positionTooltip() JS function, all 4 JS tooltip functions wired (b3ShowTip/shTip/showInstructions/_showTip), [data-tooltip]::after CSS system, data-tooltip on all 13 tab buttons, shTip() window.scrollY bug fixed. 109/109 MOT ALL CLEAR. | None |
| 73 | 2026-05-31 | v78 -> v79 | Tab bar fix ([data-tooltip] removed from position:fixed CSS rule + .tab-bar hardened), OCB Runner S11 progress feedback (parse ⏳/✅/❌ + phase badges grey/blue/green/red + progress bar + status line), HISAV 3-step save flow (①②③ state-linked buttons replace old toolbar). 109/109 MOT ALL CLEAR. | None |
| 74 | 2026-05-31 | v80 -> v81 | OCB-R: OCB Runner v4 (phase cards + ABORT button + animated parse + pulsing glow), HISAV→HITSAV rename, 7-tab top bar + sidebar restructure, GRRICE tab, Project Brain theme (:root --pb-* vars), /api/ocb/abort + /api/ocb/progress endpoints, /api/hitsav/* aliases. 109/109 MOT ALL CLEAR. | None |
| 75 | 2026-05-31 | v81 -> v82 | OCB-R: z-index scale fixed (confirm-overlay 1000→5000), timeline popup position:fixed, OCB parse 30s AbortController timeout, forgiving parser fallback, full Project Brain CSS token set, CLACR protocol (clacr_protocol.py + 4 endpoints), CLACR Relay UI, WCCS timing logs, bridge auto-post, claude_memory_snapshot.json, claude_bridge.json, loop_output 64→50. 109/109 MOT ALL CLEAR. | None |
| 76 | 2026-05-31 | v81 -> v83 | OCB-S: OCB Runner fully rebuilt — parse_ocb_block() O(n) line-scan (3-pass: sep-chars/bare-Phase/single-fallback), extract_relevant_section() scan cap 2K+early-exit, _handle_ocb_parse concurrent.futures 10s timeout, ocb2Run() 60s AbortController, Escape key abort, window.ocbAbort flag, bar colour states (amber/green/red), green terminal log. Parse test: 7-phase OCB-S in <1ms. 109/109 MOT ALL CLEAR. |
...
| 90 | 2026-06-02 | v93 -> v94 | HITSAV Copy STATUS.md button un-disabled + OCB Runner Parse/Run ghost-button styling + SpinDoctor Dark CSS theme (:root overrides + full selector block); mcc_settings.json active_theme set; 109/109 MOT ALL CLEAR | None |
| 91 | 2026-06-02 | v94 -> v95 | Toast audit: AI empty-response red toast → amber warning "Provider returned nothing — try again"; updateAiStatusBar auto-load delay 2s→10s; no /api/terminator/* endpoints found; 109/109 MOT ALL CLEAR | None |
| 92 | 2026-06-02 | v95 -> v96 | ANCOREG Observer: ancoreg_observer.py (pre_run/post_run hooks); snapshots MOT+errors before OCB, compares after, CLEAN/REGRESSION/UNKNOWN verdict → ocb_observer_log.json; terminator_feed.json on PASS; GET /api/ocb/observer-log; Observer panel in OCB Runner tab (badge + MOT±errors); 109/109 MOT ALL CLEAR | None |
